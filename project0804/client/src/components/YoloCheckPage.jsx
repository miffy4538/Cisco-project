...
export default function YoloCheckPage({
  ...
  return (
    <div style={{ padding: '24px 24px 24px 76px', background: '#0a0a0a', color: '#eee', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      <h2 style={{ marginTop: 0 }}>YOLOv8 + Node.js + Three.js 動作確認</h2>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', margin: '10px 0 20px', fontSize: 13, color: '#94a3b8' }}>
        <StatusChip label="サーバー接続" ok={connected} />
        <StatusChip label="人物検出中" ok={personCount > 0} />
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '30px', flexWrap: 'wrap' }}>
        <div>
          ...
          <video
            ...
            playsInline
            muted
            style={{ width: '480px', height: '360px', background: '#000', borderRadius: '8px', objectFit: 'cover' }}
          />
        </div>
      ...
      <div style={{ marginTop: 24, maxWidth: 900, marginInline: 'auto' }}>
        <button
          ...
          style={{ background: 'none', border: 'none', color: '#67e8f9', cursor: 'pointer', fontSize: 13 }}
        >
          {showRaw ? '▼' : '▶'} デバッグ: pose-data 生データ
        </button>
        {showRaw && (
          <pre style={{ background: '#0d1117', border: '1px solid #1f2937', borderRadius: 8, padding: 12, maxHeight: 260, overflow: 'auto', fontSize: 11, color: '#94a3b8' }}>
...
function StatusChip({ label, ok }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
      <span style={{ width: 8, height: 8, borderRadius: '50%', background: ok ? '#22d3ee' : '#475569' }} />
      {label}
    </span>
  );
}
